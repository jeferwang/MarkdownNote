...
概念：在同一个类中，允许多个同名方法，参数表不同（参数个数  参数类型 参数类型排列不同  不允许两个方法的参数表只有形参的名字不同）
...
重载的特点：
方法名称相同，与返回值类型无关，与修饰符无关，只看参数列表，且参数列表必须不同（参数个数或参数类型或顺序），调用时，根据方法参数列表的不同来区别
...
1.构造方法的方法名与类名必须完全相同    
2.不能定义返回值类型     
3.创建对象时，自动触发构造方法的调用，不可通过  .  去手动调用。    
4.注意：    
