...
如上所示，已经实现了一个下单组件，接收一个输入属性，在模板中显示出来下单的详情。

如此一来，我们的父组件（app）已经从子组件（报价组件）通过输出属性（事件）获取到了下单信息，这样，我们只需要把下单信息通过属性绑定方式绑定到另一个子组件（下单组件）即可！

说起来很复杂，其实想想也很简单，我们学了输入输出属性，那么，子组件1=输出=>父组件=输入=>子组件2，这样的模式就是中间人模式。
